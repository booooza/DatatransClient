...
class EMVCo3DAuthenticationDataAuthorizeRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['trans_status_reason']) && (mb_strlen($this->container['trans_status_reason']) > 2)) {
            $invalidProperties[] = "invalid value for 'trans_status_reason', the character length must be smaller than or equal to 2.";
        }

        if (!is_null($this->container['trans_status_reason']) && (mb_strlen($this->container['trans_status_reason']) < 2)) {
            $invalidProperties[] = "invalid value for 'trans_status_reason', the character length must be bigger than or equal to 2.";
        }

        if (!is_null($this->container['trans_status_reason']) && !preg_match("/\\d{2}/", $this->container['trans_status_reason'])) {
            $invalidProperties[] = "invalid value for 'trans_status_reason', must be conform to the pattern /\\d{2}/.";
        }
    ...
    {
        if (!is_null($trans_status_reason) && (mb_strlen($trans_status_reason) > 2)) {
            throw new \InvalidArgumentException('invalid length for $trans_status_reason when calling EMVCo3DAuthenticationDataAuthorizeRequest., must be smaller than or equal to 2.');
        }
        if (!is_null($trans_status_reason) && (mb_strlen($trans_status_reason) < 2)) {
            throw new \InvalidArgumentException('invalid length for $trans_status_reason when calling EMVCo3DAuthenticationDataAuthorizeRequest., must be bigger than or equal to 2.');
        }
        if (!is_null($trans_status_reason) && (!preg_match("/\\d{2}/", $trans_status_reason))) {
            throw new \InvalidArgumentException("invalid value for $trans_status_reason when calling EMVCo3DAuthenticationDataAuthorizeRequest., must conform to the pattern /\\d{2}/.");
        }
