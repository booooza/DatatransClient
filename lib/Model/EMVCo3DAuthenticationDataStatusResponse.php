...
class EMVCo3DAuthenticationDataStatusResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setTransStatusReason($trans_status_reason)
    {
        if (!is_null($trans_status_reason) && (mb_strlen($trans_status_reason) > 2)) {
            throw new \InvalidArgumentException('invalid length for $trans_status_reason when calling EMVCo3DAuthenticationDataStatusResponse., must be smaller than or equal to 2.');
        }
        if (!is_null($trans_status_reason) && (mb_strlen($trans_status_reason) < 2)) {
            throw new \InvalidArgumentException('invalid length for $trans_status_reason when calling EMVCo3DAuthenticationDataStatusResponse., must be bigger than or equal to 2.');
        }
        if (!is_null($trans_status_reason) && (!preg_match("/\\d{2}/", $trans_status_reason))) {
            throw new \InvalidArgumentException("invalid value for $trans_status_reason when calling EMVCo3DAuthenticationDataStatusResponse., must conform to the pattern /\\d{2}/.");
        }
